...
CURSOR_TYPES = [
    "left_ptr", "right_ptr", "pointer", "crosshair", "wait", "busy", "help",
    "text", "move", "fleur", "size_all", "size_fdiag", "size_bdiag",
    "size_hor", "size_ver", "top_side", "bottom_side", "left_side",
    "right_side", "top_left_corner", "top_right_corner",
    "bottom_left_corner", "bottom_right_corner", "forbidden", "not-allowed"
]

class XCursorGenerator(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("xcursor Theme Generator")
        self.resize(800, 500)
        ...
        self.table.setHorizontalHeaderLabels(["Image", "Name", "Type"])
    ...
    def handle_generation(self):
        ...
        for row in range(self.table.rowCount()):
            src_path = self.table.item(row, 1).data(Qt.ItemDataRole.UserRole)
            target_type = self.table.cellWidget(row, 2).currentText()
            assets.append((src_path, target_type))
